...
namespace Uniblocks
{
    ...
    public class ChunkMeshCreator : MonoBehaviour
    {
        ...
        { // returns true if a face should be spawned
            ...
            Transparency result = Engine.GetVoxelType(adjacentVoxel).VTransparency;//获取相邻cube的透明度	// get the transparency of the adjacent voxel
            ...
            {//如果当前cube为全透明
                if (result == Transparency.transparent) //相邻cube也为全透明
                    return false; //不在一个全透明cube旁边渲染另一个全透明cube // don't draw a transparent block next to another transparent block
                else //相邻cube为不透明或半透明
                    return true; //在一个不透明或半透明cube旁边渲染一个全透明cube // draw a transparent block next to a solid or semi-transparent
            }
            else
            {//当前cube为不透明或半透明
                if (result == Transparency.solid)//相邻cube为不透明
                    return false; //不在一个不透明cube旁边渲染一个不透明或半透明cube // don't draw a solid block or a semi-transparent block next to a solid block
                else //相邻cube为半透明或全透明
                    return true;//在一个半透明或全透明cube旁边渲染一个不透明或半透明cube // draw a solid block or a semi-transparent block next to both transparent and semi-transparent
            }
        }

        public bool CheckAllAdjacent(int x, int y, int z)
        { // returns true if all adjacent voxels are solid
          // 检查所有面相邻的cube是否都为不透明，true则不渲染此物体
            for (int direction = 0; direction < 6; direction++)
            {
                ...
                {//有任意一个面相邻的cube为半透明或全透明，即返回false
                    return false;
                }
            }
            return true;
        ...
        private void CreateFace(ushort voxel, Facing facing, ColliderType colliderType, int x, int y, int z)
        {
            ...
            // ==== UVs =====
            float tUnit = Engine.GetTextureUnit(voxel);
        ...
        private void UpdateMesh(Mesh mesh)
        {
            ...
            if (Engine.GenerateColliders)
            {
                ...
                {
                    vertices = SolidColliderVertices.ToArray(),
                    triangles = SolidColliderFaces.ToArray()
                };
                ;
                ...
                if (NoCollideVertices.Count > 0)
                {
                    ...
                    {
                        vertices = NoCollideVertices.ToArray(),
                        triangles = NoCollideFaces.ToArray()
                    };
                    ;
