...
namespace Uniblocks
{
    ...
    public class ChunkDataFiles : MonoBehaviour
    {
        ...
        public static Dictionary<string, string> TempChunkData; //暂存块的体素数据，等待写入文件// stores chunk's data to write into a region file later
        public static Dictionary<string, string[]> LoadedRegions; //当前读取的文件中包含的各个区域的各个块的体素数据
        // data of currently loaded regions      //LoadedRegions<regionIndex, regionData[]>  
        ...
        //解压数据，并导入到currentChunk.VoxelDate[]
        ...
        { // returns the chunk data (from memory or from file), or an empty string if data can't be found
            ...
            // try to load from TempChunkData//从暂存区读取
            ...
            // try to load from region, return empty if not found//从文件读取
        ...
        private static void WriteRegionFile(string regionIndex)
        {
            ...
            StreamWriter show_Writer = new StreamWriter(GetRegionPath_Show(Index.FromString(regionIndex)));
            int count = 0;
            foreach (string chunk in regionData)
            {
                ...
                {
                    show_Writer.Write((ushort)chunk[i]);
                    if (i % 2 == 0) { show_Writer.Write(','); }
                    else { show_Writer.Write('/'); }
                }
                if (count != regionData.Length - 1)
                {
                    writer.Write((char)ushort.MaxValue);
                    show_Writer.Write(';');
                    show_Writer.WriteLine();
                }
                count++;
            }
            show_Writer.WriteLine(count.ToString() + "个chunk");
            show_Writer.Flush();
            show_Writer.Close();
            writer.Flush();
            writer.Close();
