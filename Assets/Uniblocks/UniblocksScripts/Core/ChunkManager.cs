...
namespace Uniblocks
{
    ...
    public class ChunkManager : MonoBehaviour
    {
        ...
        public GameObject ChunkObject; // Chunk prefab//预设体

        // chunk lists
        public static Dictionary<string, Chunk> Chunks;//块序列
        private static List<Chunk> ChunkUpdateQueue; //更新队列// stores chunks ordered by update priority. Processed in the ProcessChunkQueue loop
        private static List<Chunk> ChunksToDestroy; //更新队列// chunks to be destroyed at the end of SpawnChunks

        public static int SavesThisFrame;


        // global flags
        public static bool SpawningChunks; //正在生成// true if the ChunkManager is currently spawning chunks
        public static bool StopSpawning; //停止生成// when true, the current SpawnChunks sequence is aborted (and this is set back to false afterwards)
        public static bool Initialized;//已初始化
        ...
        private void TrySpawnChunks(int x, int y, int z)
        {
            ...
            { // if we are spawning chunks already, flag to spawn again once the previous round is finished using the last requested position as origin.
              //如果正在生成块，当前一轮生成结束时，以此次请求的点为起始点开始生成
